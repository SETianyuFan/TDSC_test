...
def main():
    ...
    parser.add_argument('-d', '--data', type=str, required=False, default="/proj/berzelius-2023-99/users/x_zyang/tdsc_val/ultrasound_breast/tdsc_crop2_5",
                        help='data directory')
    parser.add_argument('-l', '--label', type=str, required=False, default='/proj/berzelius-2023-99/users/x_zyang/tdsc_val/ultrasound_breast/labels.csv',
                        help='label csv')
    ...
    parser.add_argument('-dr', '--dropout', type=float, required=False, default=0.5, help="dropout rate")
    ...
    args_directory = str(args.resize) + "_" + str(args.learningrate) + "_" \
                     + str(args.dataaugmentation) + "_" + str(args.dropout) + '_' + str(args.model) + '_' \
                         + os.path.basename(args.data) + '_fortest1'
    ...
    for i, (train_index, val_index) in enumerate(kf.split(images)):
        ...
        val_loader = DataLoader(val_ds, batch_size=8, num_workers=6, pin_memory=pin_memory)
        ...
        for epoch in range(max_epochs):
            ...
            for j in range(args.dataaugmentation):
                ...
                train_loader = DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=6, pin_memory=pin_memory)
